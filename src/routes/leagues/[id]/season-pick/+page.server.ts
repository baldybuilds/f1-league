...
import { drivers, leagues, leagueSeasons, rounds, seasonPicks } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const [[round1], [currentPick]] = await Promise.all([
		db
			.select()
			.from(rounds)
		...
		db
			.select()
			.from(seasonPicks)
			.where(and(eq(seasonPicks.leagueSeasonId, season.id), eq(seasonPicks.userId, userId)))
	]);
